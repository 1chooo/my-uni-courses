...
while True:
    ...
    if year < 2033:
        print(f"{year} 年: 月薪為 {int(round(initial_monthly_salary))} 元，該年總收入為 {int(round(total_salary))} 元")

    year += 1
    initial_monthly_salary *= (1 + annual_salary_increase)

    if year == 2033:
        print(f"直到 {year - 1} 年底，林先生在該企業的總收入為 {int(round(total_salary))} 元")

    if total_salary > standard:
        print(f"林先生工作到 {year} 年時，年收入會超過一百萬，該年收入為 {int(round(total_salary))} 元")
        break
